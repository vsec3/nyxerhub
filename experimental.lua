...
function ConvertDropdownValue(tbl)
    ...
    local TargetTransitionSpeed = 0.1 -- How fast to transition between targets
    local LostTargetTime = 0 -- Time since target was lost
    local MaxLostTargetTime = 2 -- Max time to wait before completely losing target
    local IsTargetHidden = false -- Whether current target is hidden behind wall
    
    -- Aimbot Functions
    function GetPlayerRole()
        -- Check if player is killer or survivor based on game structure
        local player = game.Players.LocalPlayer
        if not player.Character then return nil end
        
        -- Check if player is in Survivor team
        ...
        return nil -- Not in any team
    end
    
    function CanSeePlayer(targetPlayer)
        -- Check if we can see the target player (no walls in between)
        ...
        -- Cast a ray from our position to the target
        ...
        -- If raycast hits something, there's a wall in the way
    ...
    function GetClosestPlayer(role)
        ...
        -- Check if player is in the correct team folder
        ...
        -- Find players in the target team folder
        for _, player in pairs(game.Players:GetPlayers()) do
            if player ~= localPlayer and player.Character and player.Character:FindFirstChild("HumanoidRootPart") then
                -- Check if this player is in the target team folder
                if teamFolder:FindFirstChild(player.Name) then
                    -- Check if we can see this player (no walls)
    ...
    function AimbotLoop()
        ...
        AimbotConnection = game:GetService("RunService").Heartbeat:Connect(function()
            ...
            if not playerRole then return end -- Don't use aimbot if not in a team
            ...
            if CurrentTarget then
                if not CanSeePlayer(CurrentTarget) then
                    if not IsTargetHidden then
                        ...
                        LostTargetTime = LostTargetTime + (1/60) -- Assuming 60 FPS
                    end
                    
                    -- If target has been hidden for too long, lose them completely
            ...
            if targetPlayer and targetPlayer.Character and targetPlayer.Character:FindFirstChild("HumanoidRootPart") then
                ...
                if CurrentTarget ~= targetPlayer then
                    ...
                    IsTargetHidden = false
                    LostTargetTime = 0
                    -- Reset transition when switching targets
                ...
                -- Apply smoothing with target transition
                local currentCFrame = HumanoidRootPart.CFrame
                local targetCFrame = CFrame.lookAt(localPosition, localPosition + lookDirection)
                
                -- Calculate smoothing based on aimbot smoothing setting and target state
                local baseSmoothing = 1 / AimbotSmoothing
                local smoothingFactor
                
                if IsTargetHidden then
                    -- When target is hidden, use much slower smoothing to avoid snapping
                    smoothingFactor = baseSmoothing * 0.1 -- 10x slower when hidden
                ...
                -- Gradually increase transition speed for smoother switching
                ...
                -- No target found, reset current target
                CurrentTarget = nil
                IsTargetHidden = false
                LostTargetTime = 0
            end
    ...
    AnnounceGroup:AddLabel("This script was fully recoded and fixed by nexer i think, i just added cool shit and patched some bugs. Thank you for using MY (nexer + nyxus) script. ❤️", true)
    AnnounceGroup:AddDivider()
    ...
    CurrentTarget = nil -- Reset target when enabling
    IsTargetHidden = false
    LostTargetTime = 0
    AimbotLoop()
    else
    ...
    PremiumLeftGroup:AddSlider("AimbotSmoothing", {
        ...
        Callback = function(Value)
            ...
            if AimbotEnabled then
                AimbotLoop() -- Restart aimbot with new smoothing
            end
        end,
    })
    ...
    ThemeManager:SetFolder("NyxerHub")
    SaveManager:SetFolder("NyxerHub/specific-game")
    SaveManager:SetSubFolder("specific-place") -- if the game has multiple places inside of it (for example: DOORS)
    -- you can use this to save configs for those places separately
    -- The path in this script would be: NyxerHub/specific-game/settings/specific-place
    -- [ This is optional ]
